...
/*常住地*/
var provinces=$("#provinces"),citys=$("#citys"),towns=$("#towns");
for(var i=0;i<provinceList.length;i++){
    addEle(provinces,provinceList[i].name);
}
...
var provinceTexts,cityTexts,cityItems;
provinces.on("change",function(){
    provinceTexts=$(this).val();
    $.each(provinceList,function(i,item){
        if(provinceTexts == item.name){
            cityItems=i;
            return cityItems
        }
    });
    removeEle(citys);
    removeEle(towns);
    $.each(provinceList[cityItems].cityList,function(i,item){
        addEle(citys,item.name)
    })
});
